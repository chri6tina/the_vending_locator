...
export default function GuideCityPage({ params }: Params) {
  ...
  const relatedCities = stateEntry
    ? stateEntry.cities.filter(c => c.slug !== citySlug).slice(0, 3)
    : []
    ...
    <>
      ...
      {stateEntry && relatedCities.length > 0 && (
        <section className="py-12 bg-white border-t border-gray-200">
          <div className="mx-auto max-w-7xl px-6">
            <h2 className="text-xl font-playfair font-bold text-charcoal mb-4">More guides in {stateEntry.name}</h2>
            <div className="flex flex-wrap gap-3">
              {relatedCities.map(city => (
                <Link key={city.slug} href={`/how-to-start-a-vending-machine-business/${city.slug}`} className="px-3 py-2 rounded-lg border border-gray-200 bg-cream/60 text-chocolate hover:text-navy">
