...
// Comprehensive list of 500+ additional US cities/towns (20K+ population)
// These are smaller cities, suburbs, and towns not yet covered
const additionalCities = [
  // Alabama - smaller cities
  ...
  // California - many smaller cities (continuing from previous)
  ...
  // Continue with more states - I'll add more in batches to reach 500
  // Adding more from other states...
];

// Filter to only cities that don't exist
const citiesToCreate = additionalCities.filter(city => !existingPages.has(city.slug));
console.log(`Found ${citiesToCreate.length} cities to create from ${additionalCities.length} total`);

if (citiesToCreate.length < 500) {
  console.log(`\nNeed ${500 - citiesToCreate.length} more cities. Adding more from various states...`);
  
  // Add more cities from various states to reach 500
  const moreCities = [
    // More from various states - smaller cities and towns
  ...
  const additionalFiltered = moreCities.filter(city => !existingPages.has(city.slug));
  citiesToCreate.push(...additionalFiltered);
}
