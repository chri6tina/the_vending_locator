...
const howToStartDir = './src/app/how-to-start-a-vending-machine-business';
...
const citiesToCreate = missingCities.slice(0, 1000);
console.log(`\nCreating ${citiesToCreate.length} new "how-to-start" pages...\n`);

// Helper function to get state abbreviation for component name
function getStateAbbreviation(stateName) {
  ...
  return stateAbbrevs[stateName] || stateName.replace(/[^a-zA-Z]/g, '').substring(0, 2).toUpperCase();
}

// Generate component name
function getComponentName(cityName, stateName) {
  const cleanCity = cityName.replace(/[^a-zA-Z]/g, '');
  const cleanState = stateName.replace(/[^a-zA-Z]/g, '');
...
function generatePageClientTsx(citySlug, cityName, stateName) {
  ...
  // Convert to JSON strings (will be properly escaped)
  const permitInfoStr = JSON.stringify(permitInfo, null, 2);
