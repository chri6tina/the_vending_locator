...
// City-specific information database
const cityInfo = {
  'jacksonville-florida': {
    permitInfo: {
      salesTax: 'Florida requires a sales tax permit through the Department of Revenue. Jacksonville also has a local surtax. Register your business and collect/remit tax on vending sales.',
      healthPermit: 'If selling food or beverages, contact the Duval County Health Department for vending machine permits and inspection requirements.',
      businessLicense: 'Register your LLC with the Florida Division of Corporations and obtain a local business tax receipt from the City of Jacksonville.',
      additionalNotes: 'Some locations like the Port of Jacksonville or government buildings may have additional facility-specific requirements.'
    },
    demandDrivers: [
      'Jacksonville Port facilities and logistics operations',
      'Naval Air Station Jacksonville and military contractors',
      'Major healthcare systems (Mayo Clinic, Baptist Health, UF Health)',
      'Downtown office buildings and financial services',
      'Manufacturing and distribution centers',
      'University of North Florida campus',
      'Jacksonville Beach and coastal tourism areas',
      'Corporate parks in Southside and Town Center'
    ],
    neighborhoods: [
      ...
      'Westside',
      'Arlington',
      'Beaches (Jax Beach, Atlantic Beach)',
      'Riverside / San Marco',
      'Airport area'
    ]
  }
...
function getDefaultPermitInfo(cityName, stateName) {
  return {
    salesTax: `${stateName} requires a sales tax permit through the state Department of Revenue. Register your business and collect/remit tax on vending sales where applicable.`,
    healthPermit: `Contact your local county health department for food and beverage vending permits if selling consumable products in ${cityName}.`,
    businessLicense: `Register your LLC with the ${stateName} Secretary of State and obtain any required local business licenses in ${cityName}.`,
...
function getDefaultDemandDrivers(cityName) {
  return [
    `Office buildings and corporate parks in ${cityName}`,
...
function generatePageTsx(citySlug, cityName, stateName) {
  ...
  return `import ${cityName.replace(/[^a-zA-Z]/g, '')}${stateName.replace(/[^a-zA-Z]/g, '')}HowToStartPage from './pageClient'
import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'How to Start a Vending Machine Business in ${cityStateDisplay} (2025 Guide)',
...
export default function Page() {
  return <${cityName.replace(/[^a-zA-Z]/g, '')}${stateName.replace(/[^a-zA-Z]/g, '')}HowToStartPage />
}
`;
}

function generatePageClientTsx(citySlug, cityName, stateName) {
  const cityData = cityInfo[citySlug] || {};
  const permitInfo = cityData.permitInfo || getDefaultPermitInfo(cityName, stateName);
  const demandDrivers = cityData.demandDrivers || getDefaultDemandDrivers(cityName);
  const neighborhoods = cityData.neighborhoods || getDefaultNeighborhoods(cityName);
...
export default function ${cityName.replace(/[^a-zA-Z]/g, '')}${stateName.replace(/[^a-zA-Z]/g, '')}HowToStartPage() {
  const cityDisplayName = '${cityName}'
  const stateDisplayName = '${stateName}'
  const cityStateDisplay = \`\${cityDisplayName}, \${stateDisplayName}\`

  // ${cityName}-specific data
  const permitInfo = ${JSON.stringify(permitInfo, null, 4)}

  const demandDrivers = ${JSON.stringify(demandDrivers, null, 4)}

  const neighborhoods = ${JSON.stringify(neighborhoods, null, 4)}
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Link
                  href={\`/vending-leads/\${cityDisplayName.toLowerCase()}-\${stateDisplayName.toLowerCase()}\`}
                  className="inline-flex items-center justify-center px-8 py-4 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-lg transition-all duration-200 border border-white/30"
                >
...
// Main function
function createCityPage(citySlug, cityName, stateName) {
  ...
  if (!fs.existsSync(cityDir)) {
    ...
    console.log(`✅ Created directory: ${citySlug}/`);
  } else {
    console.log(`⚠️  Directory already exists: ${citySlug}/`);
  }

  // Create page.tsx
  const pageTsxPath = path.join(cityDir, 'page.tsx');
  fs.writeFileSync(pageTsxPath, generatePageTsx(citySlug, cityName, stateName));
  console.log(`✅ Created: ${citySlug}/page.tsx`);

  // Create pageClient.tsx
  const pageClientTsxPath = path.join(cityDir, 'pageClient.tsx');
  fs.writeFileSync(pageClientTsxPath, generatePageClientTsx(citySlug, cityName, stateName));
  console.log(`✅ Created: ${citySlug}/pageClient.tsx`);

  console.log(`\n✨ Successfully created "How to Start" page for ${cityName}, ${stateName}!`);
  console.log(`📍 URL: /how-to-start-a-vending-machine-business/${citySlug}\n`);
}
...
if (args.length < 3) {
  console.log('Usage: node create_how_to_start_city_pages.js <city-slug> <CityName> <StateName>');
  console.log('Example: node create_how_to_start_city_pages.js miami-florida Miami Florida');
  process.exit(1);
}

const [citySlug, cityName, stateName] = args;
createCityPage(citySlug, cityName, stateName);
