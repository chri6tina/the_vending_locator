...
// Function to extract city and state from slug
function parseCitySlug(citySlug) {
  ...
  return { cityDisplayName, stateDisplayName };
}

// Generate city data matching Auburn style
function generateCityData(citySlug) {
  const { cityDisplayName, stateDisplayName } = parseCitySlug(citySlug);
...
// Generate pageClient.tsx content (the Auburn/Tyler style)
function generatePageClient(citySlug) {
  const { cityDisplayName, stateDisplayName } = parseCitySlug(citySlug);
  const cityData = generateCityData(citySlug);
          ...
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <p className="text-stone leading-relaxed">${cityDisplayName} offers diverse vending opportunities including healthcare facilities, educational institutions, manufacturing plants, retail locations, office buildings, and transportation hubs. Each location is pre-verified for optimal vending machine success.</p>
              </motion.div>
              <motion.div
                key="1"
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                <p className="text-stone leading-relaxed">Yes, we offer comprehensive support including location research, contact information, placement strategies, and ongoing consultation to ensure your vending machines thrive in ${cityDisplayName}.</p>
              </motion.div>
              <motion.div
                key="4"
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                <p className="text-stone leading-relaxed">Healthcare, education, manufacturing, retail, office buildings, and transportation sectors in ${cityDisplayName} show the highest potential for vending machine success due to consistent foot traffic and captive audiences.</p>
              </motion.div>
              <motion.div
                key="5"
                initial={{ opacity: 0, y: 20 }}
...
// Generate page.tsx content (metadata file)
function generatePageFile(citySlug) {
  const { cityDisplayName, stateDisplayName } = parseCitySlug(citySlug);
...
// Main function to create Auburn-style city pages
function createAuburnStyleCityPage(citySlug) {
  ...
  console.log(`✅ Created Auburn-style page for ${citySlug}`);
}
...
if (require.main === module) {
  ...
  if (cities.length === 0) {
    console.log('Usage: node scripts/generate_auburn_style_city_page.js <city-slug> [more-slugs...]');
    console.log('Example: node scripts/generate_auburn_style_city_page.js orlando-florida phoenix-arizona');
    process.exit(1);
  }
  ...
  console.log(`\n✅ Created ${cities.length} Auburn-style city page(s)!`);
}

module.exports = { createAuburnStyleCityPage };
